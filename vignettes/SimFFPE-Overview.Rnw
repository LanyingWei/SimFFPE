%\VignetteIndexEntry{Introduction to SimFFPE}
...
\title{Introduction to \Biocpkg{SimFFPE}}
\author{Lanying Wei}
\date{\today}
...
genome, a Phred score profile to simulate Phred scores based on the position on the reads,
and a table of target regions (optional). The Phred score profile can be estimated from
existing BAM files using \Rfunction{estimateSimParam} function (two available
examples for Phred score profile are stored in the 'extdata' directory of
\Biocpkg{SimFFPE} package). The \Rfunction{estimateSimParam} also show messages
about some other basic statistics as read length, median / MAD of insert length
and some FFPE-related statistics. Additional information can be found on the
help page for \Rfunction{estimateSimParam}.
...
To simulate reads of certain regions, the target regions are required (not
required when simulating reads on the whole genome / several chromosomes /
large regions). The table representing the target regions should have three
columns, which indicate chromosomes, start positions and end positions
respectively (one-based coordinate).
<<target_region_table>>=
regionPath <- system.file("extdata", "regionsSim.txt", package = "SimFFPE")
targetRegions <- fread(regionPath)
@
...
targetReadSimFFPE(reference, PhredScoreProfile2, targetRegions, outFile,
coverage = 120, readLen = 100, meanInsertLen = 150, sdInsertLen = 40,
enzymeCut = FALSE)
@
